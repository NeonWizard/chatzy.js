...
class Room {
  ...
  get geozonePrefix() {
    return `${this.geozone}${this.geozoneNum}`;
  }
  ...
  assertAuthenticated() {
    ...
    if (!this.authenticated) {
      throw new Error(
        "Must be authenticated in this room to perform this operation."
      );
    }
  }
  ...
  async fetchContents() {
    ...
    const htmlRows = htmlParser
      .parse(data)
      .querySelectorAll(`#${constants.XChatBox} p`);
    const output = [];
